...
def get_complaints(
    ...
    if sort == "created":
        complaints = query.order_by(Complaint.created_at.desc()).offset(skip).limit(limit)
    else:
...
@router.get("/complaints/{id}/reply-summary", response_model=FullReplySummaryResponse)
