...
export default function AnswerSelectPage() {
  ...
  // --- Render Page ---
  return (
    // --- 디자인 수정: 전체 레이아웃 래퍼 (기존 ml-[250px] p-4 제거) ---
    <div className="bg-slate-50 min-h-screen pb-12">
      {/* --- 디자인 수정: 기존 Header -> PageHeader 컴포넌트로 변경 --- */}
      <PageHeader
        title={complaintTitle}
        icon={<FileText size={30} className="text-white drop-shadow-md" />}
        hasSidebar={true} // 사이드바가 있음을 가정
        maxWidthClass="max-w-[1000px]"
        rightContent={backButton}
      />

      {/* --- 디자인 수정: ContentCenter 래퍼 적용 (기존 p-4 max-w-[1000px] mx-auto space-y-6 대체) --- */}
      <ContentCenter hasSidebar={true} maxWidthClass="max-w-[1000px]">
        {/* --- 디자인 수정: 메인 콘텐츠 영역 스타일 (그림자, 패딩 등) --- */}
        <div className="bg-white p-6 rounded-lg shadow-lg shadow-slate-200/50 w-full space-y-6 relative mt-8">
